...
// Criando função SendPacket onde se define o tamanho dos pacotes
  ...
  Ptr<PcapFileWrapper> file = pcapHelper.CreateFile ("fifth_Trace.pcap", std::ios::out, PcapHelper::DLT_PPP);
...
// -------------------------FlowMonitor-------------------------
  ...
  //Gerar animação
  AnimationInterface anim ("fifth_Animation.xml"); // Mandatory
  ...
  anim.EnableIpv4RouteTracking ("fifth_IPV4Route.xml", Seconds (timeStartSimulation), Seconds (timeStopSimulation), Seconds (5)); //Optional
